...
pub struct PathInputs {
    ...
    pub fugue_dir_override: Option<PathBuf>,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct FuguePaths {
    pub fugue_dir: PathBuf,
...
pub fn compute_paths(inputs: PathInputs) -> FuguePaths {
    let fugue_dir = inputs
        .fugue_dir_override
        .clone()
        .unwrap_or_else(|| inputs.home_dir.join(".fugue"));

    let config_base = match inputs.fugue_dir_override {
    ...
    let socket_path = if inputs.fugue_dir_override.is_some() {
        fugue_dir.join("fugue.sock")
    } else if let Some(runtime) = inputs.xdg_runtime_dir.as_ref() {
        runtime.join("fugue.sock")
    } else {
        fugue_dir.join("fugue.sock")
    };
    ...
    FuguePaths {
        socket_path,
        pid_path: fugue_dir.join("fugue.pid"),
        log_path: fugue_dir.join("fugue.log"),
        plans_dir: fugue_dir.join("plans"),
        runtime_dir: fugue_dir.join("runtime"),
        projects_dir: fugue_dir.join("projects"),
...
mod tests {
    ...
    fn compute_paths_default() {
        let inputs = PathInputs {
            ...
            fugue_dir_override: None,
        };

        let got = compute_paths(inputs);
        assert_eq!(got.fugue_dir, PathBuf::from("/home/alice/.fugue"));
        assert_eq!(got.log_path, PathBuf::from("/home/alice/.fugue/fugue.log"));
        assert_eq!(got.config_dir, PathBuf::from("/home/alice/.config/fugue"));
        assert_eq!(
            got.socket_path,
            PathBuf::from("/home/alice/.fugue/fugue.sock")
        );
        assert_eq!(
            got.config_file,
            PathBuf::from("/home/alice/.config/fugue/config.toml")
        );
    }
    ...
    fn compute_paths_uses_xdg_config_home() {
        ...
        let got = compute_paths(inputs);
        assert_eq!(got.config_dir, PathBuf::from("/tmp/xdg/fugue"));
    }
    ...
    fn compute_paths_uses_xdg_runtime_dir_for_socket_by_default() {
        ...
        let got = compute_paths(inputs);
        assert_eq!(got.socket_path, PathBuf::from("/run/user/123/fugue.sock"));
        assert_eq!(got.fugue_dir, PathBuf::from("/home/alice/.fugue"));
    }

    #[test]
    fn compute_paths_fugue_dir_override_overrides_config() {
        let inputs = PathInputs {
            home_dir: PathBuf::from("/home/alice"),
            xdg_config_home: Some(PathBuf::from("/tmp/xdg")),
            xdg_runtime_dir: Some(PathBuf::from("/run/user/123")),
            fugue_dir_override: Some(PathBuf::from("/tmp/fugue-dev")),
        };

        let got = compute_paths(inputs);
        assert_eq!(got.fugue_dir, PathBuf::from("/tmp/fugue-dev"));
        assert_eq!(got.config_dir, PathBuf::from("/tmp/fugue-dev/config"));
        assert_eq!(got.socket_path, PathBuf::from("/tmp/fugue-dev/fugue.sock"));
    }
