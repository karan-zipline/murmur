...
mod tests {
    ...
    use crate::paths::{compute_paths, FuguePaths, PathInputs};

    fn mk_paths(tmp: &TempDir) -> FuguePaths {
        compute_paths(PathInputs {
            home_dir: tmp.path().join("home"),
            xdg_config_home: Some(tmp.path().join("xdg")),
            xdg_runtime_dir: Some(tmp.path().join("run")),
            fugue_dir_override: Some(tmp.path().join("fugue")),
        })
    }
