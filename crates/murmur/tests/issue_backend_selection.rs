...
async fn backend_selection_routes_tk_and_linear() {
    ...
    let daemon = spawn_daemon(&murmur_dir, "lin-key", &server.uri());

    let mut add_tk = cargo_bin_cmd!("mm");
    add_tk.env("MURMUR_DIR", murmur_dir.path());
    add_tk.args([
        "project",
        "add",
        "tkproj",
    ...
    let mut add_lin = cargo_bin_cmd!("mm");
    add_lin.env("MURMUR_DIR", murmur_dir.path());
    add_lin.args([
        "project",
        "add",
        "linproj",
    ...
    let mut set_team = cargo_bin_cmd!("mm");
    set_team.env("MURMUR_DIR", murmur_dir.path());
    set_team.args([
        "project",
        "config",
        "set",
    ...
    let mut ready_tk = cargo_bin_cmd!("mm");
    ready_tk.env("MURMUR_DIR", murmur_dir.path());
    ...
    let mut ready_lin = cargo_bin_cmd!("mm");
    ready_lin.env("MURMUR_DIR", murmur_dir.path());
