...
{
    let stream = UnixStream::connect(dir.path().join("murmur.sock")).await?;
    ...
    loop {
        ...
        if evt.r#type != murmur_protocol::EVT_ORCHESTRATION_TICK_REQUESTED {
            continue;
        }

        let payload: murmur_protocol::OrchestrationTickRequestedEvent =
...
async fn webhook_health_returns_ok() {
    ...
    write_webhook_config(&murmur_dir, "127.0.0.1:0", "secret");
    let bins = setup_fake_binaries();
    let daemon = spawn_daemon(&murmur_dir, &bins.path().join("bin"));

    let addr = wait_for_webhook_addr(&murmur_dir);
...
async fn github_webhook_emits_tick_requested_event() {
    ...
    wait_for_tick_event(&murmur_dir, "demo", "github", || async {
        let resp = client
            .post(url.as_str())
...
async fn linear_webhook_emits_tick_requested_event() {
    ...
    let client = reqwest::Client::new();
    wait_for_tick_event(&murmur_dir, "demo", "linear", || async {
        let resp = client
            .post(url.as_str())
