...
fn agent_done_merges_closes_and_records_recent_work() {
    ...
    let daemon = spawn_daemon(&murmur_dir, home_dir.path(), &bins.path().join("bin"));
    ...
    let mut issue = cargo_bin_cmd!("mm");
    issue.env("MURMUR_DIR", murmur_dir.path());
    ...
    let mut commits = cargo_bin_cmd!("mm");
    commits.env("MURMUR_DIR", murmur_dir.path());
    ...
    let mut stats = cargo_bin_cmd!("mm");
    stats.env("MURMUR_DIR", murmur_dir.path());
...
fn concurrent_agent_done_is_serialized_per_project() {
    ...
    let bins = setup_fake_binaries();
    let daemon =
        spawn_daemon_with_merge_delay(&murmur_dir, home_dir.path(), &bins.path().join("bin"), 250);
    ...
    let mut create2 = cargo_bin_cmd!("mm");
    create2.env("MURMUR_DIR", murmur_dir.path());
    ...
    let fugue_dir_path = murmur_dir.path().to_path_buf();
    ...
    let t1 = std::thread::spawn(move || {
        b1.wait();
        let mut done = cargo_bin_cmd!("mm");
        done.env("MURMUR_DIR", &d1);
        done.env("MURMUR_AGENT_ID", &a1);
        done.args(["agent", "done"]);
        done.assert().success().stdout("ok\n");
    });
    ...
    let t2 = std::thread::spawn(move || {
        b2.wait();
        let mut done = cargo_bin_cmd!("mm");
        done.env("MURMUR_DIR", &d2);
        done.env("MURMUR_AGENT_ID", &a2);
        done.args(["agent", "done"]);
        done.assert().success().stdout("ok\n");
    });
