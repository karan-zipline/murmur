...
fn ping_and_shutdown_work_over_ipc() {
    ...
    let mut ping = cargo_bin_cmd!("mm");
    ping.env("MURMUR_DIR", dir.path());
    ping.arg("ping");
...
async fn attach_receives_heartbeat_event() {
    ...
    murmur::ipc::jsonl::write_jsonl(&mut writer, &attach)
        .await
        .unwrap();

    let resp: murmur_protocol::Response = murmur::ipc::jsonl::read_jsonl(&mut reader)
        .await
        .unwrap()
        .unwrap();
    assert!(resp.success);
    assert_eq!(resp.r#type, murmur_protocol::MSG_ATTACH);

    let event: murmur_protocol::Event = tokio::time::timeout(Duration::from_secs(3), async {
        murmur::ipc::jsonl::read_jsonl(&mut reader)
            .await
            .unwrap()
            .unwrap()
    })
    .await
    .unwrap();
    assert_eq!(event.r#type, murmur_protocol::EVT_HEARTBEAT);
    ...
    murmur::ipc::jsonl::write_jsonl(&mut writer, &detach)
        .await
        .unwrap();
    ...
    loop {
        ...
        if msg.get("type") == Some(&serde_json::Value::String("detach".to_owned())) {
            let resp: murmur_protocol::Response = serde_json::from_value(msg).unwrap();
            assert!(resp.success);
            break;
        }
    }
