...
fn branch_cleanup_deletes_only_merged_remote_branches() {
    ...
    run_git(&seed, &["checkout", "-b", "murmur/a-merged"]);
    fs::write(seed.join("merged.txt"), "merged\n").unwrap();
    run_git(&seed, &["add", "."]);
    run_git(&seed, &["commit", "-m", "merged"]);
    run_git(&seed, &["push", "-u", "origin", "murmur/a-merged"]);

    run_git(&seed, &["checkout", "main"]);
    run_git(
        &seed,
        &["merge", "--no-ff", "murmur/a-merged", "-m", "merge"],
    );
    run_git(&seed, &["push", "origin", "main"]);

    run_git(&seed, &["checkout", "-b", "murmur/a-unmerged"]);
    fs::write(seed.join("unmerged.txt"), "unmerged\n").unwrap();
    run_git(&seed, &["add", "."]);
    run_git(&seed, &["commit", "-m", "unmerged"]);
    run_git(&seed, &["push", "-u", "origin", "murmur/a-unmerged"]);

    run_git(&seed, &["checkout", "main"]);

    let mut dry = cargo_bin_cmd!("mm");
    dry.current_dir(&seed);
    dry.args(["branch", "cleanup", "--dry-run"]);
    dry.assert()
        .success()
        .stdout(predicate::str::contains("Dry run - would delete:"))
        .stdout(predicate::str::contains("[remote]\tmurmur/a-merged"))
        .stdout(predicate::str::contains("a-unmerged").not());

    let mut clean = cargo_bin_cmd!("mm");
    ...
    let merged = Command::new("git")
        .current_dir(&seed)
        .args(["ls-remote", "--heads", "origin", "murmur/a-merged"])
        .output()
        .unwrap();
    assert!(
    ...
    let unmerged = Command::new("git")
        .current_dir(&seed)
        .args(["ls-remote", "--heads", "origin", "murmur/a-unmerged"])
        .output()
        .unwrap();
