...
pub(in crate::daemon) async fn handle_issue_list(
    ...
    let mut issues = match backend
        .list(murmur_core::issue::ListFilter::default())
        .await
    {
        Ok(v) => v,
...
pub(in crate::daemon) async fn handle_issue_create(
    ...
    let params = murmur_core::issue::CreateParams {
        title: create.title,
...
pub(in crate::daemon) async fn handle_issue_update(
    ...
    let params = murmur_core::issue::UpdateParams {
        title: update.title,
...
pub(in crate::daemon) async fn handle_issue_close(
    ...
    let payload = std::mem::take(&mut req.payload);
    let parsed: Result<murmur_protocol::IssueCloseRequest, _> = serde_json::from_value(payload);
