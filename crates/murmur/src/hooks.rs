...
use murmur_core::permissions::{evaluate_rules, Action};
use murmur_protocol::{
...
pub async fn handle_pre_tool_use(paths: &MurmurPaths) -> anyhow::Result<()> {
    ...
    let project = env::var("MURMUR_PROJECT")
        .ok()
    ...
    let agent_id = agent_id_from_env()
        .ok_or_else(|| anyhow!("MURMUR_AGENT_ID is not set (not running as a managed agent)"))?;
...
pub async fn handle_stop(paths: &MurmurPaths) -> anyhow::Result<()> {
