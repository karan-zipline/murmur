...
fn project_add_list_config_and_remove() {
    ...
    let cfg_path = fugue_dir.path().join("config").join("config.toml");
    ...
    let mut get2 = cargo_bin_cmd!("fugue");
    get2.env("FUGUE_DIR", fugue_dir.path());
    ...
    let mut list2 = cargo_bin_cmd!("fugue");
    list2.env("FUGUE_DIR", fugue_dir.path());
...
fn project_add_reuses_existing_repo_dir_after_remove() {
    ...
    let mut add_again = cargo_bin_cmd!("fugue");
    add_again.env("FUGUE_DIR", fugue_dir.path());
    add_again.args([
        "project",
        "add",
        "demo",
...
fn daemon_loads_existing_config() {
    let fugue_dir = TempDir::new().unwrap();
    let cfg_dir = fugue_dir.path().join("config");
...
fn project_add_accepts_local_repo_path_and_infers_name() {
    ...
    let repo_dir = fugue_dir.path().join("projects").join("seed").join("repo");
    assert!(repo_dir.join(".git").exists());
