...
if [[ -n "${FAKE_CLAUDE_ENV_OUT:-}" ]]; then
  printf "%s" "${FUGUE_DIR:-}" > "${FAKE_CLAUDE_ENV_OUT}"
fi
...
fn add_project(fugue_dir: &TempDir, origin: &Path) {
...
fn create_agent(fugue_dir: &TempDir) -> String {
...
fn worktree_dir(fugue_dir: &TempDir, agent_id: &str) -> PathBuf {
    fugue_dir
        .path()
        .join("projects")
        .join("demo")
...
fn claude_settings_include_shell_safe_hook_commands() {
    ...
    let settings_out = fugue_dir.path().join("claude-settings.json");
    ...
    let daemon = Command::new(assert_cmd::cargo::cargo_bin!("fugue"))
        .env("FUGUE_DIR", fugue_dir.path())
        .env("PATH", path)
        .env("FAKE_CLAUDE_SETTINGS_OUT", &settings_out)
        .env("FUGUE_HOOK_EXE", "/tmp/fugue hook exe (deleted)")
        .args(["server", "start", "--foreground"])
        .stdout(Stdio::null())
        .stderr(Stdio::null())
        .spawn()
    ...
    wait_for_daemon_ready(&fugue_dir);
    add_project(&fugue_dir, &origin);
    let agent_id = create_agent(&fugue_dir);
    ...
    assert_eq!(pre_cmd, "'/tmp/fugue hook exe' 'hook' 'PreToolUse'");
    assert_eq!(perm_cmd, "'/tmp/fugue hook exe' 'hook' 'PermissionRequest'");
    assert_eq!(stop_cmd, "'/tmp/fugue hook exe' 'hook' 'Stop'");
...
fn spawned_agents_receive_fugue_dir_env() {
    ...
    let fugue_dir = TempDir::new().unwrap();
    let env_out = fugue_dir.path().join("claude-env.txt");
    let bins = setup_fake_binaries();
    ...
    assert_eq!(got.trim(), fugue_dir.path().to_string_lossy());
...
fn hook_pre_tool_use_blocks_until_permission_respond() {
    ...
    let agent_id = create_agent(&fugue_dir);
    assert_eq!(agent_id, "a-1");
    let cwd = worktree_dir(&fugue_dir, &agent_id);
    ...
    let mut hook = Command::new(assert_cmd::cargo::cargo_bin!("fugue"))
        .env("FUGUE_DIR", fugue_dir.path())
        .env("FUGUE_AGENT_ID", &agent_id)
        .env("FUGUE_PROJECT", "demo")
    ...
    let mut respond = cargo_bin_cmd!("fugue");
    respond.env("FUGUE_DIR", fugue_dir.path());
...
async fn hook_stop_emits_agent_idle_event() {
    ...
    let socket_path = fugue_dir.path().join("fugue.sock");
    ...
    let resp: fugue_protocol::Response = read_jsonl(&mut reader).await.unwrap().unwrap();
    assert!(resp.success);

    let status = Command::new(assert_cmd::cargo::cargo_bin!("fugue"))
        .env("FUGUE_DIR", fugue_dir.path())
        .env("FUGUE_AGENT_ID", &agent_id)
        .args(["hook", "Stop"])
        .stdout(Stdio::null())
    ...
    loop {
        ...
        let evt: fugue_protocol::Event = match tokio::time::timeout(Duration::from_secs(1), async {
        ...
        if evt.r#type == fugue_protocol::EVT_AGENT_IDLE {
