...
fn plan_start_creates_plan_file_and_can_show_and_stop() {
    ...
    let plan_path = fugue_dir.path().join("plans").join("plan-1.md");
    ...
    let mut send = cargo_bin_cmd!("fugue");
    send.env("FUGUE_DIR", fugue_dir.path());
    send.args(["plan", "send-message", &plan_id, "hello"]);
    send.assert().success().stdout("ok\n");

    let mut hist = cargo_bin_cmd!("fugue");
    hist.env("FUGUE_DIR", fugue_dir.path());
...
fn manager_start_send_message_status_and_stop() {
    ...
    let mut list_agents = cargo_bin_cmd!("fugue");
    list_agents.env("FUGUE_DIR", fugue_dir.path());
...
fn manager_can_stop_and_restart_with_existing_branch() {
    ...
    let mut start_again = cargo_bin_cmd!("fugue");
    start_again.env("FUGUE_DIR", fugue_dir.path());
...
fn project_remove_delete_worktrees_removes_worktrees() {
    ...
    let mut remove = cargo_bin_cmd!("fugue");
    remove.env("FUGUE_DIR", fugue_dir.path());
    ...
    let plan_path = fugue_dir.path().join("plans").join(format!("{plan_id}.md"));
    assert!(plan_path.exists(), "plan file should be kept");
