...
async fn agent_done_pull_request_creates_pr_and_keeps_worktree() {
    ...
    if !output.status.success() {
        let mut chat = cargo_bin_cmd!("fugue");
        chat.env("FUGUE_DIR", fugue_dir.path());
        chat.args(["agent", "chat-history", &agent_id, "--limit", "50"]);
        let chat_out = chat.output().unwrap();
        let log = read_to_string_best_effort(&fugue_dir.path().join("fugue.log"));
        panic!(
            "agent done failed.\nstdout:\n{}\nstderr:\n{}\nchat:\n{}\nlog:\n{log}",
    ...
    let mut claims = cargo_bin_cmd!("fugue");
    claims.env("FUGUE_DIR", fugue_dir.path());
    ...
    assert!(
        branches.contains("remotes/origin/fugue/a-1"),
