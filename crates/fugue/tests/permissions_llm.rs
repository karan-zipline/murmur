...
fn enable_llm_checker(fugue_dir: &TempDir) {
    let mut set = cargo_bin_cmd!("fugue");
    set.env("FUGUE_DIR", fugue_dir.path());
    set.args([
        "project",
        "config",
        "set",
...
async fn hook_pre_tool_use_allows_when_llm_says_safe() {
    ...
    write_llm_config(&fugue_dir);

    let bins = setup_fake_binaries();
    let daemon = spawn_daemon(&fugue_dir, &bins.path().join("bin"), &openai.uri());

    add_project(&fugue_dir, &origin);
    enable_llm_checker(&fugue_dir);
    ...
    let output = Command::new(assert_cmd::cargo::cargo_bin!("fugue"))
