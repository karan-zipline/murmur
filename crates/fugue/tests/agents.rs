...
fn agent_lifecycle_creates_worktree_persists_runtime_and_records_chat() {
    ...
    let agents_json_path = fugue_dir.path().join("runtime").join("agents.json");
    ...
    let mut abort = cargo_bin_cmd!("fugue");
    abort.env("FUGUE_DIR", fugue_dir.path());
    ...
    let mut delete = cargo_bin_cmd!("fugue");
    delete.env("FUGUE_DIR", fugue_dir.path());
...
fn agent_tail_streams_chat_events() {
    ...
    let mut tail = Command::new(assert_cmd::cargo::cargo_bin!("fugue"))
...
fn agent_codex_backend_resumes_and_records_chat() {
    ...
    let mut send1 = cargo_bin_cmd!("fugue");
    send1.env("FUGUE_DIR", fugue_dir.path());
    ...
    let mut send2 = cargo_bin_cmd!("fugue");
    send2.env("FUGUE_DIR", fugue_dir.path());
