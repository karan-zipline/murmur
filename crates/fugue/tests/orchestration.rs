...
fn create_tk_issue(fugue_dir: &TempDir, project: &str, title: &str) -> String {
...
fn claim_list_lines(fugue_dir: &TempDir, project: &str) -> Vec<(String, String)> {
    let mut cmd = cargo_bin_cmd!("fugue");
    cmd.env("FUGUE_DIR", fugue_dir.path());
...
fn wait_for_single_claim(fugue_dir: &TempDir, project: &str) -> (String, String) {
    ...
    loop {
        ...
        let claims = claim_list_lines(fugue_dir, project);
        if claims.len() == 1 {
...
fn orchestration_spawns_next_issue_after_agent_done() {
    ...
    let issue_a = create_tk_issue(&fugue_dir, "demo", "First issue");
    let issue_b = create_tk_issue(&fugue_dir, "demo", "Second issue");
    assert!(!issue_a.is_empty());
    assert!(!issue_b.is_empty());
    assert_ne!(issue_a, issue_b);

    let mut status0 = cargo_bin_cmd!("fugue");
    status0.env("FUGUE_DIR", fugue_dir.path());
    ...
    let mut start = cargo_bin_cmd!("fugue");
    start.env("FUGUE_DIR", fugue_dir.path());
    start.args(["orchestration", "start", "demo"]);
    start.assert().success().stdout("ok\n");

    let (claimed_issue_1, agent_1) = wait_for_single_claim(&fugue_dir, "demo");
    ...
    done.env("FUGUE_AGENT_ID", &agent_1);
    done.args(["agent", "done"]);
    done.assert().success().stdout("ok\n");

    let (claimed_issue_2, agent_2) = wait_for_single_claim(&fugue_dir, "demo");
    assert_eq!(agent_2, "a-2");
    assert_ne!(claimed_issue_2, claimed_issue_1);
    assert!(claimed_issue_2 == issue_a || claimed_issue_2 == issue_b);

    let mut stop = cargo_bin_cmd!("fugue");
    stop.env("FUGUE_DIR", fugue_dir.path());
    stop.args(["orchestration", "stop", "demo"]);
    stop.assert().success().stdout("ok\n");

    let mut status1 = cargo_bin_cmd!("fugue");
    status1.env("FUGUE_DIR", fugue_dir.path());
...
fn orchestration_stop_can_abort_active_agents() {
    ...
    let _issue_a = create_tk_issue(&fugue_dir, "demo", "First issue");
    let _issue_b = create_tk_issue(&fugue_dir, "demo", "Second issue");
    ...
    let (_issue_id, agent_id) = wait_for_single_claim(&fugue_dir, "demo");
    ...
    let mut status = cargo_bin_cmd!("fugue");
    status.env("FUGUE_DIR", fugue_dir.path());
    ...
    let claims = claim_list_lines(&fugue_dir, "demo");
    assert!(
        claims.is_empty(),
