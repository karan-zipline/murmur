...
fn tk_issue_ready_close_comment_and_commit_work() {
    ...
    let daemon = spawn_daemon(&fugue_dir);
    ...
    let mut create_a = cargo_bin_cmd!("fugue");
    create_a.env("FUGUE_DIR", fugue_dir.path());
    ...
    let mut create_b = cargo_bin_cmd!("fugue");
    create_b.env("FUGUE_DIR", fugue_dir.path());
    create_b.args([
        "issue",
        "create",
        "-p",
    ...
    let mut ready = cargo_bin_cmd!("fugue");
    ready.env("FUGUE_DIR", fugue_dir.path());
    ...
    let mut comment = cargo_bin_cmd!("fugue");
    comment.env("FUGUE_DIR", fugue_dir.path());
    comment.args([
    ...
    let mut close = cargo_bin_cmd!("fugue");
    close.env("FUGUE_DIR", fugue_dir.path());
    close.args(["issue", "close", "-p", "demo", &issue_a]);
    close.assert().success().stdout("ok\n");

    let mut ready2 = cargo_bin_cmd!("fugue");
    ready2.env("FUGUE_DIR", fugue_dir.path());
    ...
    let mut commit = cargo_bin_cmd!("fugue");
    commit.env("FUGUE_DIR", fugue_dir.path());
...
fn tk_issue_project_is_detected_from_repo_cwd() {
    ...
    let mut show = cargo_bin_cmd!("fugue");
    show.env("FUGUE_DIR", fugue_dir.path());
...
fn tk_issue_list_filters_by_status() {
    ...
    let mut open = cargo_bin_cmd!("fugue");
    open.env("FUGUE_DIR", fugue_dir.path());
    ...
    let mut closed = cargo_bin_cmd!("fugue");
    closed.env("FUGUE_DIR", fugue_dir.path());
