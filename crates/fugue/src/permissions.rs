...
use fugue_core::paths::{safe_join, FuguePaths};
use fugue_core::permissions::{Action, PermissionsFile, Rule};

pub async fn load_rules(paths: &FuguePaths, project: Option<&str>) -> Result<Vec<Rule>> {
...
pub async fn load_manager_allowed_patterns(paths: &FuguePaths) -> Result<Vec<String>> {
    const DEFAULT: &[&str] = &["fugue:*"];
...
fn project_permissions_path(paths: &FuguePaths, project: &str) -> Result<PathBuf> {
...
mod tests {
    use super::*;
    use fugue_core::paths::{compute_paths, PathInputs};
    use tempfile::TempDir;

    fn test_paths(dir: &TempDir) -> FuguePaths {
        compute_paths(PathInputs {
            home_dir: dir.path().to_path_buf(),
            xdg_config_home: Some(dir.path().join("xdg")),
            xdg_runtime_dir: Some(dir.path().join("run")),
            fugue_dir_override: Some(dir.path().join("fugue")),
        })
    }
...
"#,
        ...
        assert_eq!(rules[0].action, fugue_core::permissions::Action::Deny);
        assert_eq!(rules[1].action, fugue_core::permissions::Action::Allow);
    }
    ...
    async fn missing_files_are_ok() {
        ...
        assert!(
            rules
                .iter()
                .any(|r| r.tool == "Read" && r.action == fugue_core::permissions::Action::Allow),
    ...
    async fn manager_allowed_patterns_defaults_when_missing() {
        ...
        assert!(patterns.iter().any(|p| p == "fugue:*"));
    }
...
"#,
        ...
        let patterns = load_manager_allowed_patterns(&paths).await.unwrap();
        assert_eq!(patterns, vec!["fugue:*".to_owned(), "git :*".to_owned()]);
    }
}
