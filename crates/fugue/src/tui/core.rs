...
use fugue_protocol::{AgentInfo, PermissionBehavior, PermissionRequest, UserQuestion};
...
pub struct AbortConfirm {
    pub agent_id: String,
    pub role: fugue_protocol::AgentRole,
}
...
pub struct Model {
    ...
    pub recent_commits: Vec<fugue_protocol::CommitRecord>,
    pub projects: Vec<fugue_protocol::ProjectInfo>,
...
pub enum Msg {
    ...
    AgentChatReceived(fugue_protocol::AgentChatEvent),
    AgentChatHistoryLoaded {
        agent_id: String,
        result: Result<Vec<fugue_protocol::ChatMessage>, String>,
    },
    AgentSendMessageFinished {
        agent_id: String,
        result: Result<(), String>,
    },
    StatsLoaded(Result<fugue_protocol::StatsResponse, String>),
    CommitListLoaded(Result<Vec<fugue_protocol::CommitRecord>, String>),
    ProjectListLoaded(Result<Vec<fugue_protocol::ProjectInfo>, String>),
    ...
    PlanStartFinished(Result<fugue_protocol::PlanStartResponse, String>),
                            ...
                            InputContext::Chat => {
                                ...
                                chat::append_message(
                                    ...
                                    fugue_protocol::ChatMessage {
                                        role: fugue_protocol::ChatRole::User,
        ...
        Msg::AgentSendMessageFinished { agent_id, result } => match result {
            ...
            Err(err) => {
                ...
                chat::append_message(
                    buf,
                    fugue_protocol::ChatMessage {
                        role: fugue_protocol::ChatRole::System,
...
fn role_rank(role: fugue_protocol::AgentRole) -> u8 {
    match role {
        fugue_protocol::AgentRole::Manager => 0,
        fugue_protocol::AgentRole::Planner => 1,
        fugue_protocol::AgentRole::Coding => 2,
    }
}
...
mod tests {
    ...
    fn input_enter_sends_message_and_appends_optimistically() {
        ...
        assert!(buf
            .messages
            .iter()
            .any(|m| matches!(m.role, fugue_protocol::ChatRole::User) && m.content == "hello"));
    }
    ...
    fn send_error_sets_status_and_appends_system_message() {
        ...
        assert!(buf.messages.iter().any(|m| {
            matches!(m.role, fugue_protocol::ChatRole::System) && m.content.contains("send failed")
        }));
    }
    ...
    fn question_navigation_and_submit_emits_respond_effect() {
        ...
        let req = UserQuestion {
            ...
            agent_id: "a-1".to_owned(),
            project: "demo".to_owned(),
            questions: vec![fugue_protocol::QuestionItem {
                question: "Pick one".to_owned(),
                header: "choice".to_owned(),
                multi_select: false,
                options: vec![
                    fugue_protocol::QuestionOption {
                        label: "A".to_owned(),
    ...
    fn question_other_enters_input_and_submits_freeform_answer() {
        ...
        let req = UserQuestion {
            ...
            questions: vec![fugue_protocol::QuestionItem {
                ...
                header: "choice".to_owned(),
                multi_select: false,
                options: vec![fugue_protocol::QuestionOption {
                    label: "A".to_owned(),
                    description: "Option A".to_owned(),
                }],
            }],
    ...
    fn planner_flow_picks_project_and_starts_plan() {
        let mut model = Model::new();
        model.projects = vec![fugue_protocol::ProjectInfo {
            name: "demo".to_owned(),
