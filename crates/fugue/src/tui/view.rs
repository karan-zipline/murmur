...
fn draw_header(frame: &mut Frame<'_>, model: &Model, area: ratatui::layout::Rect) {
    ...
    let running = model
        .agents
        .iter()
        .filter(|a| matches!(a.state, fugue_protocol::AgentState::Running))
        .count();
    let total = model.agents.len();

    let line = Line::from(vec![
        Span::styled("fugue", Style::default().add_modifier(Modifier::BOLD)),
        Span::raw("  "),
        conn,
        Span::raw("  "),
        Span::styled(
    ...
    if model.agents.is_empty() {
        ...
        for (idx, agent) in model.agents.iter().enumerate() {
            ...
            let (role_label, role_style) = match agent.role {
                fugue_protocol::AgentRole::Manager => (
                    "mgr",
                    Style::default()
                        .fg(Color::Magenta)
                        .add_modifier(Modifier::BOLD),
                ),
                fugue_protocol::AgentRole::Planner => (
                    "pln",
                    Style::default()
                        .fg(Color::Cyan)
                        .add_modifier(Modifier::BOLD),
                ),
                fugue_protocol::AgentRole::Coding => ("cod", Style::default().fg(Color::Gray)),
            };
            line.push(Span::styled(format!("{role_label:<3}"), role_style));
            line.push(Span::raw(" "));

            let id_style = match agent.role {
                fugue_protocol::AgentRole::Manager => Style::default()
                    .fg(Color::Magenta)
                    .add_modifier(Modifier::BOLD),
                fugue_protocol::AgentRole::Planner => Style::default()
                    .fg(Color::Cyan)
                    .add_modifier(Modifier::BOLD),
                fugue_protocol::AgentRole::Coding => Style::default().add_modifier(Modifier::BOLD),
            };
...
fn agent_state_icon(agent: &fugue_protocol::AgentInfo, now_ms: u64) -> &'static str {
    match agent.state {
        fugue_protocol::AgentState::Starting => "◌",
        fugue_protocol::AgentState::Running => spinner_frame(now_ms),
        fugue_protocol::AgentState::NeedsResolution => "!",
        fugue_protocol::AgentState::Exited => "✓",
        fugue_protocol::AgentState::Aborted => "×",
    }
}
...
fn state_style(state: fugue_protocol::AgentState) -> Style {
    match state {
        fugue_protocol::AgentState::Starting => Style::default().fg(Color::Yellow),
        fugue_protocol::AgentState::Running => Style::default().fg(Color::Cyan),
        fugue_protocol::AgentState::NeedsResolution => Style::default().fg(Color::Red),
        fugue_protocol::AgentState::Exited => Style::default().fg(Color::Green),
        fugue_protocol::AgentState::Aborted => Style::default().fg(Color::Gray),
    }
}
...
fn role_badge(role: fugue_protocol::ChatRole) -> (&'static str, Style) {
    match role {
        fugue_protocol::ChatRole::User => ("[U]", Style::default().fg(Color::Yellow)),
        fugue_protocol::ChatRole::Assistant => ("[A]", Style::default().fg(Color::Cyan)),
        fugue_protocol::ChatRole::Tool => ("[T]", Style::default().fg(Color::Magenta)),
        fugue_protocol::ChatRole::System => ("[S]", Style::default().fg(Color::Gray)),
    }
}
...
fn draw_abort_confirm_overlay(frame: &mut Frame<'_>, model: &Model, area: Rect) {
    ...
    let title = match confirm.role {
        fugue_protocol::AgentRole::Planner => format!("Stop planner {}", confirm.agent_id),
...
fn draw_permission_overlay(
    frame: &mut Frame<'_>,
    area: Rect,
    req: &fugue_protocol::PermissionRequest,
) {
    let block = Block::default()
...
fn permission_summary(req: &fugue_protocol::PermissionRequest) -> String {
...
fn draw_question_overlay(
    ...
    model: &Model,
    area: Rect,
    req: &fugue_protocol::UserQuestion,
) {
    let block = Block::default()
        .title("Question")
        .borders(Borders::ALL)
...
mod tests {
    ...
    fn renders_agents_with_state_backend_and_duration() {
        ...
        model.agents = vec![
            fugue_protocol::AgentInfo {
                id: "a-1".to_owned(),
                project: "demo".to_owned(),
                role: fugue_protocol::AgentRole::Coding,
                issue_id: "ISSUE-1".to_owned(),
                state: fugue_protocol::AgentState::Running,
            ...
            fugue_protocol::AgentInfo {
                ...
                project: "demo".to_owned(),
                role: fugue_protocol::AgentRole::Manager,
            ...
            fugue_protocol::AgentInfo {
                ...
                state: fugue_protocol::AgentState::Exited,
                created_at_ms: 0,
                backend: None,
                description: None,
                worktree_dir: "/tmp".to_owned(),
    ...
    fn renders_chat_with_role_badges_and_wrapping() {
        ...
        let mut model = Model::new();
        model.agents = vec![fugue_protocol::AgentInfo {
            id: "a-1".to_owned(),
            project: "demo".to_owned(),
            role: fugue_protocol::AgentRole::Coding,
            issue_id: "ISSUE-1".to_owned(),
        ...
        chat::append_message(
            &mut buf,
            fugue_protocol::ChatMessage {
                role: fugue_protocol::ChatRole::Assistant,
    ...
    fn renders_recent_commits_and_header_count() {
        ...
        model.recent_commits = vec![fugue_protocol::CommitRecord {
            project: "demo".to_owned(),
