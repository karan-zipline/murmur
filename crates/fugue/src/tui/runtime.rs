                ...
                match evt {
                    Some(Ok(event)) => {
                        let msg = match event.r#type.as_str() {
                            fugue_protocol::EVT_HEARTBEAT => {
                                match serde_json::from_value::<fugue_protocol::HeartbeatEvent>(event.payload) {
                                    Ok(hb) => Some(Msg::Tick { now_ms: hb.now_ms }),
                                    Err(_) => None,
                                }
                            }
                            fugue_protocol::EVT_AGENT_CHAT => {
                                match serde_json::from_value::<fugue_protocol::AgentChatEvent>(event.payload) {
                                    Ok(chat) => Some(Msg::AgentChatReceived(chat)),
                                    Err(_) => None,
                                }
                            }
                            fugue_protocol::EVT_PERMISSION_REQUEST => {
                                match serde_json::from_value::<fugue_protocol::PermissionRequest>(event.payload) {
                                    Ok(req) => Some(Msg::PermissionRequested(req)),
                                    Err(_) => None,
                                }
                            }
                            fugue_protocol::EVT_USER_QUESTION => {
                                match serde_json::from_value::<fugue_protocol::UserQuestion>(event.payload) {
