...
use axum::Router;
use fugue_core::config::WebhookConfig;
use fugue_protocol::{Event, OrchestrationTickRequestedEvent, EVT_ORCHESTRATION_TICK_REQUESTED};
...
fn resolve_project(headers: &HeaderMap, query: Option<String>) -> Result<String, &'static str> {
    ...
    if let Some(p) = header_str(headers, "X-Fugue-Project")
