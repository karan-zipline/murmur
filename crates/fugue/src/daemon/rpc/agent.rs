...
use fugue_core::agent::{AgentEvent, ChatMessage, ChatRole};
use fugue_core::commits::{CommitLog, CommitRecord as CoreCommitRecord};
use fugue_core::config::{AgentBackend, MergeStrategy};
use fugue_protocol::{
...
pub(in crate::daemon) async fn handle_agent_claim(
    ...
    let project = {
        ...
        if rt.record.role != fugue_core::agent::AgentRole::Coding {
...
pub(in crate::daemon) async fn handle_agent_done(
    ...
    if role == fugue_core::agent::AgentRole::Planner {
        let runtime = {
            let mut agents = shared.agents.lock().await;
    ...
    if role != fugue_core::agent::AgentRole::Coding {
