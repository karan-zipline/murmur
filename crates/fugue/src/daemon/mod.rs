...
use fugue_core::agent::{AgentEvent, AgentRecord, AgentRole, ChatHistory, ChatMessage, ChatRole};
use fugue_core::claims::ClaimRegistry;
use fugue_core::config::AgentBackend;
use fugue_core::paths::FuguePaths;
use fugue_core::stream::{InputMessage, MessageBody, StreamMessage};
use fugue_protocol::{AgentChatEvent, Event, EVT_AGENT_CHAT};
...
pub async fn run_foreground(paths: &FuguePaths) -> anyhow::Result<()> {
...
async fn stop_agent_runtime_keep_worktree(
    ...
    let now_ms = now_ms();
    {
        ...
        if let Some(rt) = agents.agents.get_mut(agent_id) {
            if matches!(
                rt.record.state,
                fugue_core::agent::AgentState::Starting
                    | fugue_core::agent::AgentState::Running
                    | fugue_core::agent::AgentState::NeedsResolution
            ) {
...
async fn spawn_claude_agent_process(
    ...
    .env("FUGUE_PROJECT", project)
    .env("FAB_PROJECT", project)
    .env("FUGUE_DIR", fugue_dir)
    .env("FAB_DIR", fugue_dir)
...
async fn claude_reaper(
    ...
    if rt.record.state == fugue_core::agent::AgentState::Aborted {
    ...
    loop {
        ...
        tokio::select! {
            ...
            res = reader.read_line(&mut line) => {
                ...
                match fugue_core::stream::codex::parse_stream_message_line(&line) {
                    Ok(Some(stream_msg)) => apply_stream_message(&shared, agent_id, stream_msg).await,
                    Ok(None) => {}
                    Err(err) => {
...
async fn spawn_codex_turn_process(
    ...
    cmd.env("FUGUE_AGENT_ID", agent_id)
        .env("FAB_AGENT_ID", agent_id)
        .env("FUGUE_DIR", fugue_dir)
        .env("FAB_DIR", fugue_dir)
        .current_dir(worktree_dir)
...
fn project_dir(paths: &FuguePaths, name: &str) -> std::path::PathBuf {
    paths.projects_dir.join(name)
}

async fn detect_next_agent_id_seed(paths: &FuguePaths, git: &Git) -> u64 {
    ...
    loop {
        ...
        if let Ok(refs) = git.list_refs_short(&repo_dir, "refs/heads/fugue").await {
            for r in refs {
...
fn parse_agent_num_from_branch(name: &str) -> Option<u64> {
    let rest = name.strip_prefix("fugue/a-")?;
    rest.parse::<u64>().ok()
}

async fn detect_next_plan_id_seed(paths: &FuguePaths) -> u64 {
    let mut max_seen = 0u64;
...
fn project_repo_dir(paths: &FuguePaths, name: &str) -> std::path::PathBuf {
