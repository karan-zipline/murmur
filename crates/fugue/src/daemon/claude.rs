...
mod tests {
    ...
    fn hook_exe_prefix_strips_deleted_suffix() {
        let current = Path::new("/tmp/fugue (deleted)");
        assert_eq!(
    ...
    fn hook_exe_prefix_env_override_wins() {
        let current = Path::new("/tmp/fugue");
        let override_exe = OsStr::new("/opt/fugue");
        assert_eq!(
    ...
    fn hook_exe_prefix_env_override_is_sanitized() {
        let current = Path::new("/tmp/fugue");
        let override_exe = OsStr::new(" /opt/fugue (deleted) ");
        assert_eq!(
            hook_exe_prefix_from(Some(override_exe), Some(current)),
            "/opt/fugue".to_owned()
        );
    ...
    fn render_shell_command_runs_via_sh_c_with_spacey_exe_path() {
        ...
        let exe_path = bin_dir.join("fugue-test");
        fs::write(
            &exe_path,
            r#"#!/usr/bin/env sh
set -eu
