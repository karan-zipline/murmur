...
use fugue::{client, config_store, daemon};
use fugue_core::agent::{ChatMessage, ChatRole};
use fugue_core::config::{ConfigFile, IssueBackend};
use fugue_core::paths::{compute_paths, FuguePaths, PathInputs};
...
#[command(
    name = "fugue",
    version,
    about = "Fugue — local-only agent orchestration supervisor"
)]
struct Cli {
    #[arg(long, global = true, value_name = "DIR", alias = "fab-dir")]
    fugue_dir: Option<PathBuf>,

    #[arg(long, global = true, env = "FUGUE_LOG", value_name = "LEVEL")]
...
enum IssueCommand {
    ...
    Ready,
    #[command(about = "Create a new issue. Use 'fugue issue commit' to push changes.")]
    Create {
        title: String,
    ...
    #[command(
        about = "Update an issue's status, priority, or other fields. Use 'fugue issue commit' to push changes."
    )]
    Update {
        id: String,
    ...
    #[command(about = "Mark an issue as closed. Use 'fugue issue commit' to push changes.")]
    Close { id: String },
    #[command(
...
async fn run() -> anyhow::Result<()> {
    let cli = Cli::parse();
    let paths = resolve_paths(cli.fugue_dir.as_ref())?;
    let enable_stderr_logging = !matches!(cli.command, Command::Tui);
    init_logging(&paths, cli.log_level.as_deref(), enable_stderr_logging)?;

    tracing::info!(version = env!("CARGO_PKG_VERSION"), "fugue starting");

    dispatch(cli.command, &paths).await
}

fn resolve_paths(fugue_dir_override: Option<&PathBuf>) -> anyhow::Result<FuguePaths> {
    ...
    let fugue_dir_override = fugue_dir_override
        .cloned()
        .or_else(|| env::var_os("FUGUE_DIR").map(PathBuf::from));
...
fn init_logging(
    ...
    let dir_ok = fs::create_dir_all(&paths.fugue_dir).is_ok();

    let env_level = env::var("RUST_LOG").ok();
    let level = log_level
        .map(str::to_owned)
        .or_else(|| env::var("FUGUE_LOG").ok())
        .or(env_level)
    ...
    let file_layer = if dir_ok {
        tracing_appender::rolling::RollingFileAppender::builder()
            ...
            .filename_prefix("fugue")
            .filename_suffix("log")
            .build(&paths.fugue_dir)
            .ok()
            .map(|file_appender| {
...
async fn dispatch(command: Command, paths: &FuguePaths) -> anyhow::Result<()> {
    match command {
...
fn completion(command: CompletionCommand) -> anyhow::Result<()> {
    ...
    let mut buf = Vec::new();
    generate(shell, &mut cmd, "fugue", &mut buf);
...
async fn dispatch_hook(command: HookCommand, paths: &FuguePaths) -> anyhow::Result<()> {
    match command {
        HookCommand::PreToolUse | HookCommand::PermissionRequest => {
            fugue::hooks::handle_pre_tool_use(paths).await
        }
        HookCommand::Stop => fugue::hooks::handle_stop(paths).await,
    }
}

async fn dispatch_plan(command: PlanCommand, paths: &FuguePaths) -> anyhow::Result<()> {
    match command {
...
async fn dispatch_manager(command: ManagerCommand, paths: &FuguePaths) -> anyhow::Result<()> {
    match command {
...
async fn dispatch_permission(command: PermissionCommand, paths: &FuguePaths) -> anyhow::Result<()> {
    match command {
        PermissionCommand::List => {
            let resp = client::permission_list(paths, None).await?;
            for r in resp.requests {
                let primary =
                    fugue_core::permissions::resolve_primary_field(&r.tool_name, &r.tool_input);
                println!(
                    "{}\t{}\t{}\t{}\t{}",
        ...
        PermissionCommand::Respond {
            ...
            client::permission_respond(
                paths,
                fugue_protocol::PermissionRespondPayload {
                    id: request_id,
                    behavior,
                    message: None,
                    interrupt: false,
...
async fn dispatch_question(command: QuestionCommand, paths: &FuguePaths) -> anyhow::Result<()> {
    match command {
        ...
        QuestionCommand::Respond {
            ...
            client::question_respond(
                paths,
                fugue_protocol::UserQuestionRespondPayload {
                    id: request_id,
                    answers,
                },
            )
...
fn parse_permission_behavior(s: &str) -> anyhow::Result<fugue_protocol::PermissionBehavior> {
    match s.trim().to_lowercase().as_str() {
        "allow" | "yes" | "y" => Ok(fugue_protocol::PermissionBehavior::Allow),
        "deny" | "no" | "n" => Ok(fugue_protocol::PermissionBehavior::Deny),
...
fn require_agent_id_env() -> anyhow::Result<String> {
    env::var("FUGUE_AGENT_ID")
        ...
        .map_err(|_| anyhow!("FUGUE_AGENT_ID (or FAB_AGENT_ID) environment variable not set"))
}
...
async fn dispatch_server(command: ServerCommand, paths: &FuguePaths) -> anyhow::Result<()> {
    match command {
...
async fn dispatch_project(command: ProjectCommand, paths: &FuguePaths) -> anyhow::Result<()> {
    match command {
        ProjectCommand::Add {
            input,
            name,
        ...
        ProjectCommand::Stop { project, all } => {
            ...
            for agent in resp.agents {
                ...
                if agent.role != fugue_protocol::AgentRole::Coding {
                    continue;
                }
                if matches!(
                    agent.state,
                    fugue_protocol::AgentState::Starting
                        | fugue_protocol::AgentState::Running
                        | fugue_protocol::AgentState::NeedsResolution
                ) {
...
async fn dispatch_agent(command: AgentCommand, paths: &FuguePaths) -> anyhow::Result<()> {
    match command {
        AgentCommand::Create {
            project,
            issue_id,
...
async fn dispatch_agent_plan(args: AgentPlanArgs, paths: &FuguePaths) -> anyhow::Result<()> {
    match args.command {
        ...
        None => {
            if args.prompt.is_empty() {
                return Err(anyhow!(
                    "prompt is required: fugue agent plan \"your planning task\""
                ));
            }
...
async fn agent_tail(paths: &FuguePaths, agent_id: String) -> anyhow::Result<()> {
    ...
    loop {
        ...
        if evt.r#type != fugue_protocol::EVT_AGENT_CHAT {
            continue;
        }

        let chat: fugue_protocol::AgentChatEvent =
...
fn format_agent_role(role: fugue_protocol::AgentRole) -> &'static str {
    match role {
        fugue_protocol::AgentRole::Coding => "coding",
        fugue_protocol::AgentRole::Planner => "planner",
        fugue_protocol::AgentRole::Manager => "manager",
    }
}

fn format_agent_state(state: fugue_protocol::AgentState) -> &'static str {
    match state {
        fugue_protocol::AgentState::Starting => "starting",
        fugue_protocol::AgentState::Running => "running",
        fugue_protocol::AgentState::NeedsResolution => "needs_resolution",
        fugue_protocol::AgentState::Exited => "exited",
        fugue_protocol::AgentState::Aborted => "aborted",
    }
}

fn format_chat_role(role: fugue_protocol::ChatRole) -> &'static str {
    match role {
        fugue_protocol::ChatRole::User => "user",
        fugue_protocol::ChatRole::Assistant => "assistant",
        fugue_protocol::ChatRole::Tool => "tool",
        fugue_protocol::ChatRole::System => "system",
    }
}
    ...
    match command {
        ...
        OrchestrationCommand::Stop {
            ...
            if abort_agents {
                ...
                eprintln!(
                    "note: {} agent(s) still active; stop them with `fugue agent abort <id>` or re-run with `--abort-agents`",
                    active.len()
                );
            }
...
async fn dispatch_claim(command: ClaimCommand, paths: &FuguePaths) -> anyhow::Result<()> {
    match command {
...
async fn dispatch_commit(command: CommitCommand, paths: &FuguePaths) -> anyhow::Result<()> {
    match command {
...
async fn branch_cleanup(dry_run: bool, local: bool) -> anyhow::Result<()> {
    ...
    let remote_branches = list_branches(&cwd, true)?
        .into_iter()
        .filter(|b| b.starts_with("origin/fugue/"))
        .collect::<Vec<_>>();

    let local_branches = if local {
        list_branches(&cwd, false)?
            .into_iter()
            .filter(|b| b.starts_with("fugue/"))
            .collect::<Vec<_>>()
    } else {
        Vec::new()
    };
    ...
    if merged_remote.is_empty() && merged_local.is_empty() {
        println!("No merged fugue/* branches found");
        return Ok(());
    }
...
fn determine_default_base_ref(repo_dir: &std::path::Path) -> anyhow::Result<String> {
    ...
    if let Some(branch) = fugue::git::parse_default_branch_from_remote_show(&show) {
...
async fn dispatch_issue(args: IssueArgs, paths: &FuguePaths) -> anyhow::Result<()> {
    ...
    match args.command {
        ...
        IssueCommand::Create {
            ...
            let req = fugue_protocol::IssueCreateRequest {
        ...
        IssueCommand::Update {
            ...
            let req = fugue_protocol::IssueUpdateRequest {
                project,
                id,
                title,
                description,
...
fn resolve_issue_project(
    ...
    if let Some(project) = fugue_core::project::detect_project_from_cwd(paths, config, &cwd) {
        return Ok(project);
    }
...
fn parse_issue_status(s: &str) -> anyhow::Result<fugue_protocol::IssueStatus> {
    match s.trim().to_ascii_lowercase().as_str() {
        "open" => Ok(fugue_protocol::IssueStatus::Open),
        "closed" => Ok(fugue_protocol::IssueStatus::Closed),
        "blocked" => Ok(fugue_protocol::IssueStatus::Blocked),
...
fn plan_path(paths: &FuguePaths, plan_id: &str) -> anyhow::Result<PathBuf> {
    let id = plan_id.trim().trim_end_matches(".md");
    let filename = format!("{id}.md");
    let path = fugue_core::paths::safe_join(&paths.plans_dir, &filename)
        .map_err(|e| anyhow!("invalid plan id {plan_id:?}: {e}"))?;
    Ok(path)
}

fn plan_write(paths: &FuguePaths) -> anyhow::Result<()> {
    use std::io::Read as _;
...
fn plan_list_stored(paths: &FuguePaths) -> anyhow::Result<()> {
...
async fn server_start(foreground: bool, paths: &FuguePaths) -> anyhow::Result<()> {
    if foreground {
...
async fn server_status(paths: &FuguePaths) -> anyhow::Result<()> {
...
async fn server_shutdown(paths: &FuguePaths) -> anyhow::Result<()> {
    client::shutdown(paths).await?;
    println!("ok");
    Ok(())
}

async fn ping(paths: &FuguePaths) -> anyhow::Result<()> {
    let _ = client::ping(paths).await?;
    println!("ok");
    Ok(())
}

async fn stats(paths: &FuguePaths, project: Option<String>) -> anyhow::Result<()> {
...
async fn status(paths: &FuguePaths, show_agents: bool) -> anyhow::Result<()> {
...
async fn attach(paths: &FuguePaths, projects: Vec<String>) -> anyhow::Result<()> {
    use tokio::io::{BufReader, BufWriter};
    use tokio::net::UnixStream;
    ...
    let attach = fugue_protocol::Request {
        ...
        payload: serde_json::to_value(fugue_protocol::AttachRequest { projects })
            .context("serialize attach payload")?,
    };
    write_jsonl(&mut writer, &attach)
        .await
...
async fn tui(paths: &FuguePaths) -> anyhow::Result<()> {
    fugue::tui::run(paths).await
}

fn print_event(evt: fugue_protocol::Event) -> anyhow::Result<()> {
    match evt.r#type.as_str() {
        fugue_protocol::EVT_AGENT_CHAT => {
            let chat: fugue_protocol::AgentChatEvent =
                serde_json::from_value(evt.payload).context("parse agent chat event")?;
            println!(
                "[{}:{}]\t{}\t{}",
        ...
        fugue_protocol::EVT_PERMISSION_REQUEST => {
            let req: fugue_protocol::PermissionRequest =
                serde_json::from_value(evt.payload).context("parse permission request")?;
            let primary =
                fugue_core::permissions::resolve_primary_field(&req.tool_name, &req.tool_input);
            println!(
                "[{}:{}]\tpermission\t{}\t{}",
                req.project, req.agent_id, req.tool_name, primary
            );
        }
        fugue_protocol::EVT_USER_QUESTION => {
            let req: fugue_protocol::UserQuestion =
                serde_json::from_value(evt.payload).context("parse user question")?;
            println!("[{}:{}]\tquestion\t{}", req.project, req.agent_id, req.id);
        }
        fugue_protocol::EVT_AGENT_IDLE => {
            let agent_id = evt
                .payload
                .get("agent_id")
                .and_then(|v| v.as_str())
        ...
        fugue_protocol::EVT_ORCHESTRATION_TICK_REQUESTED => {
            let tick: fugue_protocol::OrchestrationTickRequestedEvent =
